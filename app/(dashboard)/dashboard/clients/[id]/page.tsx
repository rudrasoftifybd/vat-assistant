    ...
    async function loadClient() {
      ...
      if (data) {
        setForm({
          ...
          annual_turnover: data.annual_turnover?.toString() || "",
          registration_date: data.registration_date || "",
          vat_registered: data.vat_registered ?? true,
        });
      }
      setLoading(false);
    }
  ...
  return (
    <div className="space-y-8 max-w-2xl mx-auto">
      <div className="flex items-center gap-4">
        ...
        <h1 className="text-[34px] font-[600] tracking-[-0.374px]">{t("clients.edit_client", language)}</h1>
      </div>

      <Card className="bg-[var(--canvas)] border border-[var(--hairline)] rounded-[18px]">
        <CardHeader className="p-[24px] pb-0">
          <CardTitle className="text-[11px] font-[600] uppercase tracking-[0.5px] text-[var(--ink-muted-48)]">{t("clients.edit_client", language)}</CardTitle>
        </CardHeader>
