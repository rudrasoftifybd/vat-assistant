...
import { Plus, FileDown, ShoppingCart, Pencil, Trash2 } from "lucide-react";
...
export default function PurchasesPage() {
  ...
  async function loadData() {
    ...
    const { data } = await supabase.from("register_entries").select("*").eq("org_id", profile.org_id).eq("type", "purchase").order("entry_date", { ascending: false });
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const payload = { org_id: profile.org_id, client_id: form.client_id || null, type: "purchase", entry_date: form.entry_date, party_name: form.party_name, party_bin: form.party_bin || null, product_name: form.product_name, quantity: qty, unit_price: price, total_price: qty * price, vat_amount: vat, invoice_ref: form.invoice_ref || null };
  ...
  const handleExport = () => {
    try {
      ...
      XLSX.utils.book_append_sheet(wb, ws, "ক্রয় রেজিস্টার");
      XLSX.writeFile(wb, "purchase_register.xlsx");
  ...
  return (
    <div className="max-w-[1440px] mx-auto space-y-8">
      <div className="flex items-center justify-between">
        <h1 className="text-[34px] font-[600] tracking-[-0.374px]">{t("registers.purchase", language)}</h1>
      ...
      <Card className="bg-[var(--canvas)] border border-[var(--hairline)] rounded-[18px]"><CardContent className="p-0">
        <Table><TableHeader><TableRow>
          <TableHead>{t("registers.purchase_date", language)}</TableHead>
          <TableHead>{t("registers.supplier_name", language)}</TableHead>
          <TableHead>{t("registers.supplier_bin", language)}</TableHead>
        ...
        <TableBody>
          {entries.length === 0 ? (
            <TableRow><TableCell colSpan={9} className="py-16 text-center text-[17px] text-[var(--ink-muted-48)]"><ShoppingCart className="mx-auto h-12 w-12 mb-4 opacity-50" />{t("common.no_data", language)}</TableCell></TableRow>
          ) : entries.map((e) => (
            <TableRow key={e.id}>
      ...
      <Dialog open={showForm} onOpenChange={(v) => { setShowForm(v); if (!v) setEditingEntry(null); }}>
        ...
        <form onSubmit={handleSubmit} className="space-y-6 mt-4">
          ...
          <div className="bg-[var(--canvas)] border border-[var(--hairline)] rounded-[18px] p-[24px]">
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-2"><Label className="text-[13px] text-[var(--ink-muted-80)] tracking-[-0.224px]">{t("registers.purchase_date", language)}</Label><Input type="date" value={form.entry_date} onChange={(e) => setForm({ ...form, entry_date: e.target.value })} required /></div>
              ...
              <div className="space-y-2"><Label className="text-[13px] text-[var(--ink-muted-80)] tracking-[-0.224px]">{t("registers.supplier_name", language)}</Label><Input value={form.party_name} onChange={(e) => setForm({ ...form, party_name: e.target.value })} required /></div>
              <div className="space-y-2"><Label className="text-[13px] text-[var(--ink-muted-80)] tracking-[-0.224px]">{t("registers.supplier_bin", language)}</Label><Input value={form.party_bin} onChange={(e) => setForm({ ...form, party_bin: e.target.value })} /></div>
